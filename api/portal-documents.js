...
const FILE       = 'data/portal_documents.json';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    if (session.role === 'customer') docs = docs.filter(d => d.customerId === session.customerId && d.active);
  ...
  if (req.method === 'POST') {
    ...
    if (!title || !customerId || !fileUrl) return res.status(400).json({ error: 'title, customerId and fileUrl required' });
    const rec = { id: crypto.randomUUID(), customerId, assetId: assetId || null, title, type: type || 'Other', fileUrl, uploadedAt: new Date().toISOString(), active: active !== false };
    docs.push(rec);
    await ghPut(FILE, docs, sha);
